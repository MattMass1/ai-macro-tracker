...
final class AppStore: ObservableObject {
    ...
    func refreshAfterVoice() async {
        // Voice actions operate on today unless the user explicitly says otherwise.
        // Keep the day view, saved plan, set history, and progress in sync.
        selectedDate = Date()
        async let current: Void = loadAll()
        async let history: Void = workoutHistory()
        async let progress: Void = trends(days: trendsWindow)
        _ = await (current, history, progress)
    }
