...
/// Identifies the credential used by a failed request without passing the
/// credential itself through notifications. Late failures cannot end a new login.
struct DeviceTokenRejection {
    private let digest: Data

    init(token: String) { digest = Data(SHA256.hash(data: Data(token.utf8))) }

    func matches(token: String?) -> Bool {
        guard let token, !token.isEmpty else { return false }
        return digest == Data(SHA256.hash(data: Data(token.utf8)))
    }

    static func post(token: String) {
        NotificationCenter.default.post(name: .deviceTokenRejected, object: DeviceTokenRejection(token: token))
    }
}
...
final class APIClient {
    ...
    private func request<T: Decodable>(_ path: String, method: String, body: Data?, authenticated: Bool = true) async throws -> T {
        ...
        let requestToken = authenticated ? tokenProvider() : nil
        if authenticated {
