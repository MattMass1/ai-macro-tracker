...
final class LiveCoachWebSocketTransport: NSObject, LiveCoachTransporting {
    ...
    func connect() async throws -> AsyncThrowingStream<LiveCoachServerEvent, Error> {
        let requestToken = tokenProvider()?.trimmingCharacters(in: .whitespacesAndNewlines)
        let request = try Self.makeRequest(baseURL: baseURL, token: requestToken)
        ...
        do {
            ...
            cleanUpSocket()
            if status == 401, let requestToken {
                DeviceTokenRejection.post(token: requestToken)
            }
            throw Self.connectionError(status: status)
        }
    ...
    static func connectionError(status: Int?) -> LiveCoachConnectionError {
        ...
        case 429:
            return LiveCoachConnectionError(message: "Voice is busy. Wait a moment, then reconnect.", retryable: true)
        case 502, 503, 504:
            return LiveCoachConnectionError(message: "The voice server is starting or unavailable. Try again shortly.", retryable: true)
        default:
            return LiveCoachConnectionError(message: "Voice coach could not connect. Check your connection and try again.", retryable: true)
        }
    }
