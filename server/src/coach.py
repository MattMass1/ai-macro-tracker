...
TOOLS = [
    ...
    _schema("lookup_food", "Look up real macros using the verified catalog, curated menus, FatSecret when configured, and OpenFoodFacts fallback. When a food is not a saved preset or known food and macros are needed, call this FIRST, then cite the returned source string as macro_source.", {"query": S}, ("query",)),
    ...
    _schema("set_workout_plan", """Write a workout plan built from library exercises. The server adds the version automatically. STABLE ROUTINE: The stored plan is the user's routine. NEVER rewrite, regenerate, replace, or 'improve' the plan on your own. Only call set_workout_plan during onboarding (to create the FIRST plan) or when the user explicitly asks to change it. If the user mentions changing or swapping a specific exercise, make the minimal edit they asked for — do not redesign the whole day. Do not proactively suggest changing exercises, and do not rotate/swap exercises for variety. Use EXACTLY this shape:
...
- STABLE ROUTINE: The stored plan is the user's routine. NEVER rewrite, regenerate, replace, or 'improve' the plan on your own. Only call set_workout_plan during onboarding (to create the FIRST plan) or when the user explicitly asks to change it. If the user mentions changing or swapping a specific exercise, make the minimal edit they asked for — do not redesign the whole day. Do not proactively suggest changing exercises, and do not rotate/swap exercises for variety.
